/* ============================================================
   galaxy-charge.js — FUSION: <galaxy-cluster> × hold-to-charge
   <galaxy-charge> — full-screen living galaxy cluster with a
   Neon-style hold-to-charge detonation mechanic and a
   full-charge "ENTER THE WIKI" state.

   Particle engine: DAN_SITE/galaxy-cluster.js
     (@danfrank · THE CLUSTER · 2026-06-12 · Claude Design output)
   Hold-to-charge bomb: Neon_Scrolling_Messages
     ("Radioactive Transmissions" · dc-runtime demo · 2024–2026)
   Fusion: Sammy · 2026-09-19
   ...
   A central black hole wears the `text` attribute as a particle
   constellation; spiral arms, satellite galaxies and energy
   streams orbit it.

   Interaction model:
     idle         — galaxy drifts, cursor = gravity well
     drag         — grab + flick matter
...
(function () {
  'use strict';
  if (window.__galaxyChargeDefined) return;
  window.__galaxyChargeDefined = true;
  ...
  var VS_UPDATE = [
    ...
    'uniform float uSuck;',                  /* fusion: core implosion while charging */
    'uniform vec2 uShock;',
    ...
    '    spring = 9.0; damp = 3.2;',
    '  } else if (type > 0.5) {',
    '    /* wordmark constellation: heavy — stiff spring, faint shimmer, shrugs off every force */',
    '    home = aHomeCtr.xy + 0.004 * vec2(sin(uTime * 1.8 + ph * 37.0), cos(uTime * 1.3 + ph * 21.0));',
    '    spring = 60.0; damp = 7.0;',
    '  } else {',
    ...
    '  float wm = (type > 0.5 && type < 1.5) ? 1.0 : 0.0;  /* wordmark: heavy, barely feels forces */',
    '  /* magnetic cursor gravity */',
    '  vec2 md = uMouse - pos;',
    '  float mr = length(md) + 1e-4;',
    '  vec2 mdir = md / mr;',
    '  acc += mdir * uHover * 38.0 * exp(-mr * 2.6) * mix(1.0, 0.06, wm);',
    '  /* hold = spiral vortex: spin + suck */',
    '  float vfall = exp(-mr * 1.5) * uVortex;',
    '  acc += (vec2(-mdir.y, mdir.x) * 210.0 + mdir * 70.0) * vfall * mix(1.0, 0.02, wm);',
    '  /* fusion: implosion toward the core while the breach charges */',
    '  float cr = length(pos) + 1e-4;',
    '  vec2 cdir = pos / cr;',
    '  float cfall = exp(-cr * 1.4) * uSuck;',
    '  acc += (-cdir * (38.0 * cfall) + vec2(cdir.y, -cdir.x) * (95.0 * cfall)) * mix(1.0, 0.02, wm);',
    '  /* click = expanding shockwave ring */',
    '  vec2 sd = pos - uShock;',
    '  float sr = length(sd) + 1e-4;',
    '  float ring = exp(-pow((sr - uShockT * 2.4) * 7.0, 2.0)) * exp(-uShockT * 2.6) * uShockAmp;',
    '  acc += (sd / sr) * ring * 620.0 * mix(1.0, 0.03, wm);',
    '  vel += acc * dt;',
    '  /* flick / throw momentum from cursor velocity */',
    '  vel += uMouseVel * exp(-mr * 8.0) * (dt * 60.0) * 0.09 * mix(1.0, 0.1, wm);',
    '  vel *= exp(-damp * dt);',
  ...
  var FS_BG = [
    ...
    'uniform float uIntensity;',
    'uniform vec2 uHold;',    /* fusion: charge point, world coords */
    'uniform float uRing;',   /* fusion: charge-ring visibility 0..1 */
    'out vec4 fragColor;',
    ...
    '  /* fusion: hold-charge ring around the pointer, grows with charge */',
    '  float hd = length(uv - uHold);',
    '  float rr = 0.08 + uCharge * 0.34;',
    '  float band = exp(-abs(hd - rr) * 34.0) * uRing;',
    '  vec3 ringC = mix(vec3(0.0, 0.94, 1.0), vec3(1.0, 0.176, 0.584), clamp(uCharge, 0.0, 1.0));',
    '  col += ringC * band * (0.55 + 0.45 * sin(uTime * 9.0));',
    '  fragColor = vec4(col, 1.0);',
    '}'
  ].join('\n');

  /* fusion: overlay program that renders the "ENTER THE WIKI" text texture */
  var VS_TXT = [
    '#version 300 es',
    'layout(location=0) in vec2 aPos;',
    'out vec2 vUv;',
  ...
  var FS_TXT = [
    ...
    '  fragColor = vec4(t.rgb * (uAlpha * (0.72 + 0.28 * uPulse)), 1.0);',
    '}'
  ].join('\n');
  ...
  function compileShader(gl, type, src) {
    ...
    if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
      console.error('[galaxy-charge] shader: ' + gl.getShaderInfoLog(sh));
      return null;
    }
    return sh;
  }
  ...
  function makeProgram(gl, vsSrc, fsSrc, tfVaryings) {
    ...
    if (!gl.getProgramParameter(p, gl.LINK_STATUS)) {
      console.error('[galaxy-charge] link: ' + gl.getProgramInfoLog(p));
      return null;
    }
    var u = {};
  ...
  class GalaxyCharge extends HTMLElement {
    static get observedAttributes() { return ['intensity', 'text', 'sub', 'entertext']; }

    constructor() {
      super();
      this._intensity = 1; this._tintensity = 1;
      this._text = 'WIKI BRAIN';
      this._sub = 'HOLD TO ENTER';
      this._enterText = 'ENTER THE WIKI';
    }
    ...
    attributeChangedCallback(name, _o, v) {
      ...
      if (name === 'intensity') {
        ...
        this._text = String(v);
        if (this._st) this._buildWordmark();
      } else if (name === 'sub') {
        this._sub = String(v);
        if (this._st) this._buildWordmark();
      } else if (name === 'entertext') {
        this._enterText = String(v);
        if (this._gl && !this._2d) this._buildEnterTexture();
      }
    }
    ...
    connectedCallback() {
      ...
      var t0 = this.getAttribute('text');
      if (t0 != null && t0 !== '') this._text = t0;
      var s0 = this.getAttribute('sub');
      if (s0 != null) this._sub = s0;
      var e0 = this.getAttribute('entertext');
      ...
      this._suck = 0;                     /* fusion: core implosion 0..1 */
      this._ring = 0;                     /* fusion: charge-ring visibility 0..1 */
      ...
      if (gl) {
        this._prog = {
          ...
          txt: makeProgram(gl, VS_TXT, FS_TXT)
        };
        if (!this._prog.update || !this._prog.render || !this._prog.bg || !this._prog.txt) this._gl = gl = null;
      }
      if (gl) {
        ...
        this._buildEnterTexture();
      } else {
        console.warn('[galaxy-charge] WebGL2 unavailable — using Canvas2D fallback');
      ...
      this._onUp = function (e) { self._prelease(e); };
      this._onOut = function (e) { if (!e.relatedTarget) { self._hoverT = 0; self._prelease(); } };
    ...
    _sizeAll() {
      ...
      if (this._gl) {
        /* ENTER-text quad: undistorted, ~1.56 NDC wide */
        var gl = this._gl;
        if (this._txtBuf) gl.deleteBuffer(this._txtBuf);
        if (this._vaoTxt) gl.deleteVertexArray(this._vaoTxt);
        var qw = 0.78;
        var qh = qw * (256 / 1024) * (this._c.width / this._c.height);
        var buf = this._txtBuf = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, buf);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array([-qw, -qh, qw, -qh, -qw, qh, qw, qh]), gl.STATIC_DRAW);
        var va = this._vaoTxt = gl.createVertexArray();
    ...
    _buildParticles(preserve) {
      ...
      var textCount = this._textCount = Math.floor(N * 0.22);
    ...
    /* fusion: rasterize the ENTER text into a GL texture once */
    _buildEnterTexture() {
      var gl = this._gl;
      if (!gl) return;
      var cw = 1024, ch = 256;
      ...
      try { x.letterSpacing = '18px'; } catch (e) { /* older canvas */ }
      x.font = '900 118px "Arial Black", Arial, sans-serif';
      x.shadowColor = 'rgba(0,240,255,0.95)';
      x.shadowBlur = 42;
      x.fillStyle = 'rgba(180,255,255,1)';
      x.fillText(this._enterText, cw / 2, ch / 2);
      x.shadowColor = 'rgba(255,45,149,0.9)';
      x.shadowBlur = 18;
      x.fillStyle = '#ffffff';
      x.fillText(this._enterText, cw / 2, ch / 2);
      if (this._enterTex) gl.deleteTexture(this._enterTex);
      var tex = this._enterTex = gl.createTexture();
    ...
    _buildWordmark() {
      if (!this._st) return;
      var label = this._text, sub = this._sub;
      var w = 1200, h = 520;
      var o = document.createElement('canvas');
      o.width = w; o.height = h;
      ...
      x.font = '900 ' + fs + 'px Unbounded, sans-serif';
      x.fillStyle = '#fff';
      x.fillText(label, w / 2, h / 2 - fs * 0.16);
      x.font = '700 ' + (fs * 0.19) + 'px "IBM Plex Mono", monospace';
      x.fillText(sub, w / 2, h / 2 + fs * 0.62);
      ...
      /* stride across ALL sampled pixels: the wordmark budget (a fixed share
         of N) is usually smaller than the sample count, and the samples arrive
         in top-to-bottom scan order — taking the first textCount of them cut
         the bottom half of every glyph off. Striding covers the full height. */
      var stride = n / this._textCount;
    ...
    _pdown(e) {
      ...
      /* fusion: no instant shock — the release detonates, scaled by charge.
         keep the original breach-feed tick so near-core stirs still charge it. */
    ...
    /* original protocol: only matter near the core feeds the breach */
    _addCharge(p, amt) {
      if (!isFinite(amt) || !isFinite(p.x) || !isFinite(p.y)) return;
      if (Math.sqrt(p.x * p.x + p.y * p.y) > 0.55) return;
      if (this._charge >= 1) return;
      this._setCharge(this._charge + amt, true);
    }

    /* fusion: the one writer for charge. Emits 'void:charge' on change
       (throttled to 0.015 steps + the 0/1 edges) so hosts can listen. */
    _setCharge(v, force) {
      v = Math.max(0, Math.min(1, v));
      this._charge = v;
      if (force || v === 0 || v === 1 || Math.abs(v - this._emitCache) > 0.015) {
        this._emitCache = v;
    ...
    _enter() {
      ...
      var self = this;
      setTimeout(function () { self._enterTarget = 0; self._entered = false; }, 1400);
    }
    ...
    _frame(tms) {
      ...
      /* fusion: hold charges the breach to full over ~2s, wherever held */
      if (this._down) this._setCharge(this._charge + dt / 2.0);

      /* spinning the vortex over the core also feeds it */
      if (this._vortex > 0.4) this._addCharge({ x: this._mx, y: this._my }, 0.004);

      /* fusion: smoothed charge-driven visuals */
      this._suck += ((this._down ? this._charge : 0) - this._suck) * Math.min(1, dt * 6);
      this._ring += ((this._down ? 1 : 0) - this._ring) * Math.min(1, dt * 9);
      this._enterAlpha += (this._enterTarget - this._enterAlpha) * Math.min(1, dt * 5);
    ...
    _frameGPU(t, dt) {
      ...
      gl.uniform1f(P.u.uSuck, this._suck);
      ...
      /* 2 — nebula backdrop (+ hold-charge ring) */
      ...
      gl.uniform1f(P.u.uIntensity, this._intensity);
      gl.uniform2f(P.u.uHold, this._mx, this._my);
      gl.uniform1f(P.u.uRing, this._ring);
      gl.bindVertexArray(this._vaoBg);
      ...
      /* 4 — fusion: ENTER THE WIKI text overlay, additive */
      var ea = this._enterAlpha;
      if (ea > 0.004 && this._enterTex) {
        P = this._prog.txt;
        gl.useProgram(P.prog);
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, this._enterTex);
        gl.uniform1i(P.u.uTex, 0);
        gl.uniform1f(P.u.uAlpha, ea);
        gl.uniform1f(P.u.uPulse, 0.5 + 0.5 * Math.sin(t * 7.0));
        gl.bindVertexArray(this._vaoTxt);
    ...
    _frameCPU(t, dt) {
      ...
      /* fusion: hold-charge ring around the pointer */
      if (this._ring > 0.01) {
        var hrx = (this._mx / A * 0.5 + 0.5) * w;
        var hry = (0.5 - this._my * 0.5) * h;
        var hrr = (0.08 + this._charge * 0.34) * syw;
        var rc = neonRGB(this._charge * 0.25);
        ctx.strokeStyle = 'rgba(' + rc[0] + ',' + rc[1] + ',' + rc[2] + ',' +
          (0.75 * this._ring * (0.6 + 0.4 * Math.sin(t * 9.0))) + ')';
        ctx.lineWidth = 2 + 5 * this._charge;
        ctx.beginPath();
        ctx.arc(hrx, hry, hrr, 0, 6.2831853);
        ctx.stroke();
      }
      ...
      var dampDisc = Math.exp(-1.6 * dt), dampText = Math.exp(-7.0 * dt), dampStream = Math.exp(-3.2 * dt);
      ...
      for (var i = 0; i < N; i++) {
        ...
        if (type > 1.5) {
          ...
          /* wordmark: heavy — stiff spring, faint shimmer, shrugs off every force */
          hx = st[o8] + 0.004 * Math.sin(t * 1.8 + ph * 37);
          hy = st[o8 + 1] + 0.004 * Math.cos(t * 1.3 + ph * 21);
          spring = 60; damp = dampText;
        } else {
        ...
        var wm = (type > 0.5 && type < 1.5) ? 1 : 0;  /* wordmark: heavy, barely feels forces */
        ...
        var pull = hover * 38 * Math.exp(-mr * 2.6) * (wm ? 0.06 : 1);
        accx += mdirx * pull; accy += mdiry * pull;
        if (vortex > 0.001) {
          var vfall = Math.exp(-mr * 1.5) * vortex;
          accx += (-mdiry * 210 + mdirx * 70) * vfall * (wm ? 0.02 : 1);
          accy += (mdirx * 210 + mdiry * 70) * vfall * (wm ? 0.02 : 1);
        }
        /* fusion: implosion toward the core while charging */
        if (suck > 0.001) {
          var cr = Math.sqrt(px * px + py * py) + 1e-4;
          var cdx = px / cr, cdy = py / cr;
          var cfall = Math.exp(-cr * 1.4) * suck;
          accx += (-cdx * 38 * cfall + cdy * 95 * cfall) * (wm ? 0.02 : 1);
          accy += (-cdy * 38 * cfall + -cdx * 95 * cfall) * (wm ? 0.02 : 1);
        }
        if (shockEnv > 0.001) {
          ...
          accx += (sdx / sr) * ring * 620 * (wm ? 0.03 : 1);
          accy += (sdy / sr) * ring * 620 * (wm ? 0.03 : 1);
        }
        vx += accx * dt; vy += accy * dt;
        var fl = Math.exp(-mr * 8) * flickK * (wm ? 0.1 : 1);
        vx += mvx * fl; vy += mvy * fl;
        vx *= damp; vy *= damp;
      ...
      /* fusion: ENTER THE WIKI state on the 2D canvas */
      if (this._enterAlpha > 0.01) {
        ctx.save();
        ctx.globalAlpha = this._enterAlpha * (0.72 + 0.28 * (0.5 + 0.5 * Math.sin(t * 7.0)));
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.font = '900 ' + Math.max(24, Math.round(h * 0.085)) + 'px "Arial Black", Arial, sans-serif';
        ctx.shadowColor = 'rgba(0,240,255,0.95)';
        ctx.shadowBlur = 34;
        ctx.fillStyle = '#ffffff';
        ctx.fillText(this._enterText, w / 2, h / 2);
        ctx.restore();
      }
    }
  }

  customElements.define('galaxy-charge', GalaxyCharge);
})();
